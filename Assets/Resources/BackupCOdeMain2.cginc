...
void calcFinalColor(inout Ray ray, inout ColData Color, const HitMat hitDat, RayHit hit, uint2 Uv, inout bool terminated, float2 NormalUV, bool HitVoxel) {//main function
   ...
   // return;
    [branch]if(UseNEE || AllowVolumetrics) {//Next event estimation
        ...
        if(UseUnityLight) {
            triindex = CurrentReservoir[Uv.x + Uv.y * screen_width].y;
...
bool validbsdf;
    ...
    //   if(prevReservoir.y == lighttosample && (prevReservoir.WasSuccessful == 0)) continue;
        ...
        float distance_to_light_squared = dot(L, L);
        L /= sqrt(distance_to_light_squared);
        float3 LightNorm = normalize(WorldPosition - light.Position);
        float cos_theta_light = abs(dot(L, LightNorm));
        float cos_theta_hit = dot(L, Norm);
        float3 Radiance = light.Radiance;
        float light_pdf = luminance(Radiance.x, Radiance.y, Radiance.z) * distance_to_light_squared / (cos_theta_light * (_UnityLights[unitylightcount - 1].TotalEnergy));
            
        float3 bsdf_value;
        float bsdf_pdf;
        validbsdf = evaldiffuse(L, cos_theta_hit, bsdf_value, bsdf_pdf);
        float w = power_heuristic(light_pdf, bsdf_pdf) / light_pdf;//1.0f cuz its a point light
    ...
    LightData light = _UnityLights[reservoir.y];
        ...
        float bsdf_pdf;
        validbsdf = evaldiffuse(L, cos_theta_hit, bsdf_value, bsdf_pdf);
        float p_hat = power_heuristic(light_pdf, bsdf_pdf) / light_pdf;//1.0f cuz its a point light
    ...
    //    _DebugTex[id.xy] = float4(bsdf_value, 1);
...
bool validbsdf;
    ...
    LightData light = _UnityLights[r.y];
        ...
        validbsdf = evaldiffuse(L, cos_theta_hit, bsdf_value, bsdf_pdf);
        float w = power_heuristic(light_pdf, bsdf_pdf) / light_pdf;//1.0f cuz its a point light
    UpdateReservoir(reservoirNew, r.y, w * r.W * r.M, float(rand_xorshift()) * (1.0 / 4294967296.0));
    ...
    for (int i = 0; i < 100;i++)
    {
        ...
        float radius = 30 * float(rand_xorshift()) * (1.0 / 4294967296.0);
    ...
    //  Exceed 10% of current pixel's depth
        ...
        validbsdf = evaldiffuse(L, cos_theta_hit, bsdf_value, bsdf_pdf);
        float w = power_heuristic(light_pdf, bsdf_pdf) / light_pdf;//1.0f cuz its a point light
         UpdateReservoir(reservoirNew, neighborRes.y, w * neighborRes.W * neighborRes.M, float(rand_xorshift()) * (1.0 / 4294967296.0));
    ...
    light = _UnityLights[reservoirNew.y];
        ...
        distance_to_light_squared = dot(L, L);
        L /= sqrt(distance_to_light_squared);
        LightNorm = normalize(WorldPosition - light.Position);
        cos_theta_light = abs(dot(L, LightNorm));
        cos_theta_hit = dot(L, Norm);
        ...
        bsdf_pdf;
        validbsdf = evaldiffuse(L, cos_theta_hit, bsdf_value, bsdf_pdf);
        w = power_heuristic(light_pdf, bsdf_pdf) / light_pdf;//1.0f cuz its a point light
    if(w == 0)
        reservoirNew.W == 0;
    
    else
        reservoirNew.W = (1.0 / max(w, 0.00001)) * (reservoirNew.wsum / max(reservoirNew.M, 0.0001));
