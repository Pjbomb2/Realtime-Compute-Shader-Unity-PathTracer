...
rng_state = pixel_index * curframe;
    ...
    {
        if(PreviousReservoir[prevIndex.y * screen_width + prevIndex.x].WasSuccessful == 1) prevReservoir = PreviousReservoir[prevIndex.y * screen_width + prevIndex.x];
    }
bool validbsdf;
    ...
    _DebugTex[id.xy] = float4(reservoir.y / 4.0f, 0, 0, reservoir.W);
...
bool validbsdf;
    ...
    LightData light = _UnityLights[r.y];
        ...
        validbsdf = evaldiffuse(L, cos_theta_hit, bsdf_value, bsdf_pdf);
        float w = power_heuristic(light_pdf, bsdf_pdf) / light_pdf;//1.0f cuz its a point light
    UpdateReservoir(reservoirNew, r.y, w * r.W * r.M, random(34).x);
    ...
    //  Exceed 10% of current pixel's depth
        ...
        validbsdf = evaldiffuse(L, cos_theta_hit, bsdf_value, bsdf_pdf);
        float w = power_heuristic(light_pdf, bsdf_pdf) / light_pdf;//1.0f cuz its a point light
         UpdateReservoir(reservoirNew, neighborRes.y, w * neighborRes.W * neighborRes.M, random(i + 100).y);
