...
float3 PointLightPBRRaytrace(LightData light, float3 normal, float3 worldPos, float3 cameraPos, float roughness, float metalness, float3 surfaceColor, float3 f0)
{
    ...
    return ((kd * surfaceColor.xyz / PI) + specular) * lambert * atten * light.Radiance;
...
float disneyBrdfDiffuseFactor(float cosIn, float cosOut, float cosInHalf, float roughness, float metallic) {
    ...
    float fresnelDiffuse = lerp(1.0, fresnelDiffuse90, fresnelIn) * lerp(1.0, fresnelDiffuse90, fresnelOut);
    return fresnelDiffuse * (1.0f - metallic) / PI;
}
float3 disneyBrdfDiffuse(float cosIn, float cosOut, float cosInHalf, float3 albedo, float roughness, float metallic) {
...
float evaluatePHat(
    float3 worldPos, float3 lightPos, float3 normal, float3 lightNormal, bool useLightNormal,
    ...
    wi /= sqrt(sqrDist);
    float3 wo = normalize(float3(camPos) - worldPos);
    ...
    return emissionLum *  disneyBrdfDiffuseLuminance(cosIn, cosOut, cosInHalf, albedoLum, roughness, metallic) * geometry;
}
...
void UpdateReservoir(inout Reservoir r, float x, float w, float rndnum)
{
    r.wsum += w;
    r.M += 1;
    if (rndnum < (w/max(r.wsum,0.00001f)))
        r.y = x;
...
rng_state = pixel_index * curframe;
    ...
    for(int i = 0; i < min(LightCount, 32); i++) {
        ...
        float w = evaluatePHat(WorldPosition, light.Position, Norm, normalize(light.Position - WorldPosition), false, AlbedoLum, light.energy, metallic, metallic) / p;
    ...
    float3 bsdf_value;
        ...
        float p_hat = evaluatePHat(WorldPosition, light.Position, Norm, normalize(light.Position - WorldPosition), false, AlbedoLum, light.energy, metallic, metallic);
...
void kernel_reservoir_spatial (uint3 id : SV_DispatchThreadID) {//Moved final accumulation into its own kernel, improves performance
    ...
    float p_hat = evaluatePHat(WorldPosition, light.Position, Norm, normalize(light.Position - WorldPosition), false, AlbedoLum, light.energy, metallic, metallic); // technically p_hat is divided by pdf, but point light pdf is 1
    ...
    p_hat = evaluatePHat(WorldPosition, light.Position, Norm, normalize(light.Position - WorldPosition), false, AlbedoLum, light.energy, metallic, metallic); // technically p_hat is divided by pdf, but point light pdf is 1
